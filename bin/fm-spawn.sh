...
# primary firstmate checkout (FM_ROOT) is ALSO a distinct git toplevel of the
# very same repo whenever a task targets firstmate's own repo, and a freshly
# created pane can transiently report that path before treehouse's `cd`
# lands, so it would otherwise be wrongly accepted. Git already distinguishes
# a repo's main worktree from its linked ones: the main worktree's
# --git-dir IS its --git-common-dir, while every worktree treehouse hands out
# is a linked worktree whose --git-dir is a `<common-dir>/worktrees/<id>`
# subdirectory. Matching --git-common-dir against the project's own further
# confirms the candidate belongs to the TARGET project's repository, not some
# other one. --path-format=absolute keeps the comparison meaningful: without
# it, git reports the main worktree's --git-dir as the relative ".git" but a
# linked worktree's as an absolute path, so a naive string compare would
# always differ regardless of ownership.
...
validate_spawn_worktree() {  # <source> <inspect-target>
  ...
  # Orca owns and creates its own worktree directly (no treehouse pool involved),
  # so the treehouse-ownership signal below only applies to the treehouse path.
  if [ "$BACKEND" != orca ] && ! is_linked_worktree_of "$PROJ_ABS" "$WT"; then
    echo "error: $source resolved '$WT', which is not a treehouse-managed linked worktree of $PROJ_ABS (it is that repo's main worktree, or belongs to a different repository); refusing to launch to avoid tangling the primary checkout. Inspect target $inspect_target" >&2
    exit 1
  fi
}
...
if [ "$KIND" != secondmate ] && [ "$BACKEND" != orca ]; then
  ...
  # "Distinct from the project dir" alone is not enough acceptance criteria:
  # the primary firstmate checkout (FM_ROOT) is itself a distinct git toplevel
  # that can transiently show in the pane before treehouse's own `cd` lands,
  # so also require is_linked_worktree_of to confirm the candidate is a real
  # treehouse-managed linked worktree of THIS project, not the project's own
  # main/primary checkout (data/learnings.md 2026-07-06/07-09).
  # FM_SPAWN_WORKTREE_POLL_SLEEP_SECS (default 1s, test-only override) shortens
  # this loop's real wall-clock cost for a permanently-invalid pane path, which
  # now runs the full 60 iterations before timing out instead of failing on the
  # first poll - the necessary cost of no longer accepting the first merely-
  # distinct path (see comment above).
  for _ in $(seq 1 60); do
    p=$(spawn_current_path "$WT_TARGET" || true)
    if [ -n "$p" ] && [ "$(real_path_or_raw "$p")" != "$PROJ_ABS_REAL" ] && is_linked_worktree_of "$PROJ_ABS" "$p"; then
      WT="$p"
      break
    fi
    sleep "${FM_SPAWN_WORKTREE_POLL_SLEEP_SECS:-1}"
  done
  if [ -z "$WT" ]; then
...
if [ "$KIND" != secondmate ]; then
  case "$HARNESS" in
    claude*)
      mkdir -p "$WT/.claude"
      # Merge into any existing settings.local.json rather than overwriting the
      # whole file: a stray write to the wrong directory (e.g. the worktree-
      # detection race above landing on the primary checkout) must never be able
      # to wipe out that file's unrelated permissions/hooks content - defense in
      # depth alongside the worktree-detection fix itself (data/learnings.md,
      # 2026-07-09). Only .hooks.Stop is set/replaced; every other top-level key,
      # including other hook events, is preserved untouched.
      SETTINGS="$WT/.claude/settings.local.json"
      STOP_HOOK_CMD="touch '$TURNEND'"
      if command -v jq >/dev/null 2>&1; then
        EXISTING='{}'
        if [ -s "$SETTINGS" ] && jq empty "$SETTINGS" >/dev/null 2>&1; then
          EXISTING=$(cat "$SETTINGS")
        fi
        printf '%s' "$EXISTING" | jq --arg cmd "$STOP_HOOK_CMD" \
          '.hooks.Stop = [{"hooks":[{"type":"command","command":$cmd}]}]' \
          > "$SETTINGS.tmp" && mv "$SETTINGS.tmp" "$SETTINGS"
      elif [ -e "$SETTINGS" ]; then
        echo "warning: jq not found; leaving existing $SETTINGS untouched rather than risking its other settings, so this crewmate's turn-end Stop hook was not installed (supervision falls back to stale-pane detection)." >&2
      else
        cat > "$SETTINGS" <<EOF
