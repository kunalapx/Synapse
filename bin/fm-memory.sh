...
#   propose corroborate confirm promote conflicts resolve recall sweep-stale
#
...
fm_regen_index() {
  ...
  tmp=$(mktemp)
  {
    ...
    printf 'promote/resolve/sweep-stale. Do not hand-edit.\n\n'
  } > "$tmp"
  if [ -d "$ENTRIES" ]; then
    for f in "$ENTRIES"/*.md; do
      [ -e "$f" ] || continue
...
main() {
  local verb=${1:-}
  [ -n "$verb" ] || die "usage: fm-memory.sh <propose|corroborate|confirm|promote|conflicts|resolve|recall|sweep-stale> [options]" 2
  shift
  case "$verb" in
    propose) cmd_propose "$@" ;;
