...
# --- worktree-detection must reject a foreign git toplevel, not just "distinct" ---
#
# A freshly created pane's pane_current_path can transiently report some OTHER
# unrelated git repository's checkout before treehouse's own `cd` lands (a
# tmux/PTY startup race reporting the pane's pre-`-c` starting directory - a
# race independently confirmed in this operator's local fleet notes, not
# tracked in this repo). The old wait-loop's acceptance test was only "a git
# toplevel distinct from the project dir", which wrongly accepted ANY such
# unrelated checkout, including - worst case - the very primary checkout this
# isolation guard exists to protect. This drives the real bin/fm-spawn.sh
# against a fake tmux whose first pane_current_path poll returns a foreign,
# unrelated repository's checkout and whose second poll onward returns the
# real treehouse-added worktree of the SAME project repo, and asserts
# fm-spawn.sh keeps polling past the foreign path and only accepts the
# genuine linked worktree of the target project.
make_spawn_foreign_path_fakebin() {  # <dir> <foreign-path> <worktree-path> -> echoes fakebin dir
  local dir=$1 foreign=$2 wt=$3 fb="$1/fakebin" counter="$1/poll-count"
  mkdir -p "$fb"
  : > "$counter"
  cat > "$fb/tmux" <<SH
#!/usr/bin/env bash
...
test_spawn_rejects_foreign_git_toplevel_before_treehouse_worktree() {
  local proj wt foreign id fb data state config log out rc
  proj="$TMP_ROOT/foreignpath-proj"; wt="$TMP_ROOT/foreignpath-wt"; foreign="$TMP_ROOT/foreignpath-foreign"
  id="spawnforeignpath"
  fm_git_worktree "$proj" "$wt" "fm/$id"
  fm_git_init_commit "$foreign"
  fb=$(make_spawn_foreign_path_fakebin "$TMP_ROOT/foreignpath-fake" "$foreign" "$wt")
  data="$TMP_ROOT/foreignpath-data"; mkdir -p "$data/$id"
  printf 'test brief content\n' > "$data/$id/brief.md"
  state="$TMP_ROOT/foreignpath-state"; config="$TMP_ROOT/foreignpath-config"
  mkdir -p "$state" "$config"
  log="$TMP_ROOT/foreignpath-spawn.log"

  out=$(run_spawn_case "$ROOT" "$fb" "$log" "$state" "$data" "$config" "$proj" -- "$id" "$proj" claude 2>&1)
  rc=$?
  expect_code 0 "$rc" "fm-spawn.sh should succeed once the fake pane settles into the real treehouse worktree"$'\n'"$out"
  assert_contains "$out" "worktree=$wt" \
    "fm-spawn.sh did not resolve to the real treehouse worktree after a foreign git toplevel showed transiently"
  assert_not_contains "$out" "worktree=$foreign" \
    "fm-spawn.sh wrongly accepted an unrelated foreign git repository's checkout as the crewmate's worktree"
  assert_grep "worktree=$wt" "$state/$id.meta" \
    "fm-spawn.sh recorded the wrong worktree= in state/$id.meta"

  rm -rf "/tmp/fm-$id"
  pass "fm-spawn.sh: a foreign git toplevel transiently reported by the pane is rejected in favor of the real treehouse-managed worktree"
}

# --- claude turn-end hook install must merge, not overwrite -----------------
#
# The claude-harness turn-end hook installer used to `cat >` the worktree's
# .claude/settings.local.json unconditionally, so a worktree-detection false
# accept (the exact bug covered above) could silently wipe out unrelated
# content already at that path - observed for real as a clobbered captain
# compliance permission rule. This seeds a settings.local.json with an
# unrelated permissions key and a different hook event before spawning, then
# asserts both survive alongside the freshly installed Stop hook.
test_spawn_claude_hook_merges_into_existing_settings() {
  command -v jq >/dev/null 2>&1 || { pass "fm-spawn.sh claude hook merge skipped without jq"; return; }
  local proj wt id fb data state config log out rc settings
  proj="$TMP_ROOT/hookmerge-proj"; wt="$TMP_ROOT/hookmerge-wt"
  id="spawnhookmerge"
  fm_git_worktree "$proj" "$wt" "fm/$id"
  settings="$wt/.claude/settings.local.json"
  mkdir -p "$wt/.claude"
  printf '%s' '{"permissions":{"allow":["Bash(git *)"]},"hooks":{"PreToolUse":[{"matcher":"Bash","hooks":[{"type":"command","command":"echo pre"}]}]}}' \
    > "$settings"
  fb=$(make_spawn_fakebin "$TMP_ROOT/hookmerge-fake" "$wt")
  data="$TMP_ROOT/hookmerge-data"; mkdir -p "$data/$id"
  printf 'test brief content\n' > "$data/$id/brief.md"
  state="$TMP_ROOT/hookmerge-state"; config="$TMP_ROOT/hookmerge-config"
  mkdir -p "$state" "$config"
  log="$TMP_ROOT/hookmerge-spawn.log"

  out=$(run_spawn_case "$ROOT" "$fb" "$log" "$state" "$data" "$config" "$proj" -- "$id" "$proj" claude 2>&1)
  rc=$?
  expect_code 0 "$rc" "fm-spawn.sh should succeed while merging into an existing settings.local.json"$'\n'"$out"
  jq -e '.permissions.allow == ["Bash(git *)"]' "$settings" >/dev/null \
    || fail "fm-spawn.sh clobbered pre-existing permissions in settings.local.json"$'\n'"$(cat "$settings")"
  jq -e '.hooks.PreToolUse[0].hooks[0].command == "echo pre"' "$settings" >/dev/null \
    || fail "fm-spawn.sh clobbered a pre-existing, unrelated hook event in settings.local.json"$'\n'"$(cat "$settings")"
  jq -e '.hooks.Stop[0].hooks[0].command | contains("turn-ended")' "$settings" >/dev/null \
    || fail "fm-spawn.sh did not install the Stop turn-end hook into settings.local.json"$'\n'"$(cat "$settings")"

  rm -rf "/tmp/fm-$id"
  pass "fm-spawn.sh: installing the claude turn-end Stop hook merges into an existing settings.local.json instead of overwriting it"
}
