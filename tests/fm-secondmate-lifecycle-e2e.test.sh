...
#   - seeding never touches no-mistakes state for any cloned project (the
#     retired validation-pipeline mechanism has no init step left to run)
...
phase_seed() {
  ...
  # Retired mechanism: seeding never runs or references no-mistakes for any clone.
  assert_absent "$SUB/projects/gamma/.no-mistakes-init" "seed unexpectedly ran no-mistakes init in the subhome"
  assert_absent "$SUB/projects/gamma/.no-mistakes-doctor" "seed unexpectedly ran no-mistakes doctor in the subhome"
  ...
  pass "seed: registry scope+projects, charter copied, clones+origins, no leftover no-mistakes init"
}
