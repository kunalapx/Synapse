...
description: >-
  Agent-only playbook for coordinating visible Codex Desktop threads alongside Synapse without pretending they are a selectable shell backend.
  Use before creating, reading, steering, archiving, debugging, or reviewing a Codex App visible thread for Synapse work, and before responding to requests to make Codex App native to Synapse.
...
Use this playbook when Synapse work needs a visible Codex Desktop thread.
...
Codex Desktop visible threads are companion host-tool workflows, not a selectable Synapse backend.
...
If local helper scripts exist for Codex App work, use only helpers explicitly provided by the operator or maintained by Synapse.
...
3. Do not create projectless threads for repo work.
   If the project is absent, stop and ask for the project to be added or use a normal Synapse backend instead.
4. Decide whether this is a real Synapse-managed task or a visible companion thread.
...
A Desktop-owned Codex thread can append to Synapse status files only when the prompt gives an absolute path and the Desktop permission context can write that checkout.
...
Do not claim it is a complete Synapse backend.
...
For Synapse reconciliation, prefer concrete evidence:
...
If there is a real Synapse task record, leave teardown decisions to the normal Synapse task flow instead of this skill.
