...
description: Self-update a running firstmate and its domain agents to the latest from origin. Use when the boss invokes /updatefirstmate (e.g. "/updatefirstmate", "update firstmate", "pull the latest firstmate"). Fast-forwards this firstmate repo's default branch and every domain agent home from origin (fast-forward only, never forced, never disruptive), then re-reads AGENTS.md and nudges each updated domain agent to do the same, so the whole tree runs the latest bin/ and instructions.
user-invocable: true
metadata:
  internal: true
---
...
This skill performs that pull for the running main firstmate and every domain agent, without disturbing any in-flight work.

The update is **fast-forward only** - the same sanctioned self-write as the agent pool sync firstmate already runs.
...
1. **Run the updater:**
   ...
   bin/fm-update.sh
   ```
   It fast-forwards this firstmate repo's default branch from origin, then fast-forwards every registered domain agent home (each a treehouse worktree of this same repo, leased at a detached HEAD on the default branch) the same way.
...
4. **Report to the boss in plain outcomes.**
   Summarize what landed without firstmate's internal vocabulary: which parts of the agent pool are now on the latest, and which were left as-is and why.
   For example: "Boss, firstmate and both domain supervisors are now on the latest."
