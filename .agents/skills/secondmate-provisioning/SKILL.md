...
description: >-
  Agent-only reference for persistent domain agent setup and retirement.
  Use when creating, seeding, validating, launching, recovering, handing backlog to, pushing inherited local material into, or retiring a domain agent home, or when editing data/secondmates.md.
  Covers local leases, whole-home remote routes, transactional seeding, record intake for an existing or inherited domain, project clone restrictions, domain agent harness pins, inherited local-material push, idle charter, handoff helper, and teardown safety.
...
Use this reference before creating, seeding, validating, launching, handing backlog to, recovering, pushing inherited local material into, or retiring a persistent domain agent, and before editing `data/secondmates.md`.

Keep the always-inline routing rules in `AGENTS.md` authoritative: route by natural-language `scope:`, local-only projects stay with the main Synapse, and domain agents are idle by default.

## Routing table

`data/secondmates.md` has one parser-compatible line per persistent domain agent.
A local route uses:
...
For a remote route, `host:` is an OpenSSH config alias and `root:` is that host's separate tracked Synapse code root.
A remote second-mate agent always runs on the Herdr backend and every seed, launch, and liveness relaunch first gates its host on `bin/fm-remote-doctor.sh` readiness, so an unready host refuses with that doctor's own gap text rather than half-creating a route; the workers that domain agent supervises keep the home's ordinary backend selection.
This release places whole domain agent homes remotely and never individual workers.
...
Scaffold a domain agent charter with:

```sh
bin/fm-brief.sh <id> --secondmate {<project>...|--no-projects}
```

The scaffold writes a charter task spec instead of a task spec.
...
Pass `--no-projects` instead of a project list to scaffold a project-less charter for a domain whose subject is the Synapse repo itself, whose home is a Synapse worktree and whose agents take pooled worktrees of the same repo.
...
You resolve each project's origin yourself - from the boss, the project registry, a clone that exists elsewhere, `gh-axi`, or an explicit paste - and name it as `<project>=<origin-url>`; the seed validates and transports what you supply.
A remote seed therefore creates nothing in this home beyond the route, the charter task spec, and a launch record once it is launched: never clone a project into `projects/` or run an agent pool sync just to seed a remote domain agent.
...
`-` durably leases a fresh Synapse worktree via `treehouse get --lease` under the domain agent id.
...
`bin/fm-home-seed.sh` copies the charter into the domain agent home as `data/charter.md`.
...
`bin/fm-spawn.sh --secondmate` launches it through the domain agent harness path, resolving `config/secondmate-harness` -> `config/crew-harness` -> the primary's own harness unless an explicit per-spawn harness override is passed.

`config/secondmate-harness` may also pin a concrete model and effort for the domain agent agent, in the SAME file rather than a new one: the format is a single whitespace-separated line `<harness> [<model>] [<effort>]`, with only the first non-empty, non-comment line parsed.
...
For a `--secondmate` spawn, `bin/fm-spawn.sh` populates `MODEL`/`EFFORT` from those tokens only when the harness itself came from the domain agent config path for that spawn.
...
Because this resolves from the file on every spawn, the pin is durable across every respawn (recovery, `/updatefirstmate`, restart) exactly like the harness axis itself - e.g. `config/secondmate-harness` containing `claude opus` keeps a domain agent pinned to Opus even if the primary's own default model later changes.
This is domain-agent-only: agent/research task model resolution is untouched by this file.

This section is the single owner of the domain agent sync and inherited-local-material propagation contract; `AGENTS.md` sections 3 and 4 point here.
Before a local launch, `fm-spawn.sh --secondmate` locally fast-forwards the home to the primary Synapse checkout's current default-branch commit when it is safe; dirty, diverged, or in-flight homes launch unchanged with a warning.
The locked session-start deferred network stage runs the same bootstrap sweep for every live local domain agent home, discovered from `state/<id>.meta` records with `kind=secondmate` (`data/secondmates.md` only backfills `home=` for older records).
That no-fetch path is a purely local fast-forward of tracked files, never an origin fetch, and it never touches the gitignored operational dirs, so a domain agent's backlog, projects, and in-flight work are never disturbed; a linked worktree advances immediately, while a standalone clone that lacks the target receives Synapse updates through `/updatefirstmate`'s origin refresh.
...
Propagation failures warn without blocking domain agent launch or session-start continuation, and the destination keeps whatever safely validated state the helper left behind.
Inheritance copies the literal `config/crew-harness` file, so a domain agent's own agents use the primary's agent harness only when it names a concrete adapter such as `codex`; an unset or `default` value has nothing concrete to inherit, and the domain agent's own agents fall back to the domain agent's own or detected harness instead.
Inherited `config/backend` becomes that domain agent home's local runtime-backend default for future spawns only; it never retargets, rewrites, migrates, stops, or restarts an already-live worker endpoint.
A present primary value always converges byte-exact into validated domain agent homes, and primary absence removes the destination so those homes keep runtime auto-detection.
...
`config/secondmate-harness` is not inherited because it is only the primary's knob for launching domain agent agents.
`data/captain-shared.md` is main-authoritative in the primary home and read-only in domain agent homes.
Its primary file header must state that the file is main-authoritative, read-only in domain agent homes, must not be edited there, and that new boss-preference discoveries are routed to the main Synapse through marked status or a document pointer.
Every propagation point converges the domain agent copy to the primary bytes; when the primary file is absent, any existing domain agent copy is quarantined and removed so absence converges too.
...
Between propagation runs, the domain agent copy is filesystem read-only; the helper may make its owned destination writable only around a guarded update and restores read-only mode on success, unchanged bytes, and recoverable failure paths.
Before replacing divergent domain agent bytes, the helper hash-compares source and destination, quarantines the domain-agent-local version to a collision-safe private dated sibling file, and emits a `SECONDMATE_SYNC:` diagnostic naming the home and quarantine artifact.
Never copy any domain agent `data/captain-shared.md` back into the primary.
...
Keep every `data/learnings.md` fully local by boss decision; route agent-pool-general machinery facts into tracked documentation through the normal Synapse repo path rather than inventing shared learnings propagation.
...
A separate, literal-content config reread is required whenever inherited `config/*` material changes under an already-running domain agent.
...
A newly launched or relaunched domain agent already reads its files at launch, so its pending config-reread generations are discarded or quarantined after cleanup failure and it needs no redundant live-agent config nudge unless propagation changes files after launch.
...
For already-live domain agents, use `bin/fm-config-push.sh` to push a mid-session inherited local-material change without running the tracked-file fast-forward.
...
Direct seed without a preexisting task spec requires `FM_SECONDMATE_CHARTER`.
...
If validation, cloning, or registry update fails, generated task specs, new homes, new project clones, and registry edits are rolled back.

Domain agent project lists may include `direct-PR` projects only.
`local-only` projects stay with the main Synapse.
...
When a domain agent is created for a domain, existing main-backlog items that fall under its scope should become its work instead of staying stranded in the main backlog.
Scope-matching is Synapse's judgment against the domain agent's natural-language scope, not a keyword rule.
...
After seeding, run this handoff for the new domain agent's in-scope queued items.
...
For a local route, the helper resolves and validates the domain agent home from `data/secondmates.md`, then delegates the item move to `tasks-axi mv` (the single owner of the backlog format), which moves each named item - and a whole connected set, blocker plus dependents, atomically - from the main `data/backlog.md` into the domain agent home's `data/backlog.md`.
...
After a new local placement or a remote outbox receipt becomes durable, the helper sends one marked routed-work instruction through the receiving domain agent's recorded endpoint; missing or failed delivery makes the command fail loudly with the moved work intact, and the same handoff command retries known-undelivered wake intent without moving an already-present item again.
...
This delegated route remains required when `config/backlog-backend=manual`, which controls only routine Synapse backlog edits.
...
It is idempotent; an item already in the domain agent backlog is skipped.
...
That same rule governs routing generally: a Relay-linked request whose work goes to a domain agent cannot use the home-local mention link at all and needs a promised-final commitment bound to that domain agent's home.
It refuses any destination that is not a genuine seeded Synapse home with safe operational directories and a matching `.fm-secondmate-home` marker, so a move can never land in a project.
Do not hand off `local-only` items.

## Recovery

For local `kind=secondmate` meta with no window, treat the domain agent as a dead persistent direct report and respawn it with:
...
If meta is missing but `data/secondmates.md` still registers the domain agent, respawn from the registry entry and its persistent home.
...
`stuck-crewmate-recovery`'s remote-domain agent note owns why the endpoint-dead and send-failed verdicts that seem to justify this are themselves unreliable.
Respawn re-resolves the domain agent harness from current config, uses the same guarded pre-launch sync, and re-propagates inherited local material, so recovered domain agents converge inherited config items and shared boss preferences whenever their home validates; tracked-file sync remains guarded separately.
If the domain agent is already running and only inherited local material changed, prefer `bin/fm-config-push.sh` over respawning.
To move a live LOCAL domain agent onto a newly pinned harness, model, or effort without a full recovery, set `config/secondmate-harness` and then relaunch it with `bin/fm-control.sh <id> relaunch`, which re-resolves that pin, stops the agent, and launches the replacement in the same home ([`docs/agent-control.md`](../../../docs/agent-control.md)).
That plane refuses a remotely placed domain agent by name, because its agent runs on another host where none of the plane's postconditions can be read; use the remote route's own relaunch path for those.

Do not reconstruct a domain agent's whole tree from the main home.
The main Synapse reconciles only direct reports.
Each domain agent is a Synapse in its own home, so it runs recovery on startup and reconciles its own agents.
A domain agent's recovery reconciles only work that is already its own and then idles.
...
Run `bin/fm-teardown.sh <id>` for `kind=secondmate` only when the boss or main Synapse explicitly decides to retire that persistent domain agent.

The safety check is the domain agent's own home.
...
When safe, teardown kills the direct endpoint, removes the `data/secondmates.md` route, clears the main home metadata, and removes the retired domain agent home.
...
It kills child windows, discards child work and state inside the domain agent home, removes the route, releases the lease, and removes the retired domain agent home.
