---
name: stow
description: Sweep the current session for uncaptured durable knowledge, file it to disk, persist the open work records this session knows are unfiled or now wrong, and curate the home's tiered, decaying startup memory before a context reset. Use when the boss invokes /stow (e.g. "/stow", "stow what you've learned"), before a session reset or context compaction, or periodically to keep operational memory current.
user-invocable: true
metadata:
  internal: true
---
...
This skill writes only through the existing Synapse ownership and write boundaries.
...
- `<!--a:YYYY-MM-DD/N-->` - only in a home that has opted in to the pass horizon below: either dated marker may carry `/N`, the number of passes that evaluated the entry without reinforcing it.
  An absent `/N` means zero, so an entry the agent pool keeps exercising costs no counter bytes at all, and a home that has not opted in never writes one.
...
- `pinned` - no clock is ever read for it: exempt from decay and from budget eviction, changed only through inspect-then-update when the boss or reality changes it, except that an explicit per-item boss approval may offload it under the flow below.
...
In a home that stows daily those two rates diverge by the stow cadence, an entry the agent pool keeps exercising never sits unreinforced for 30 wall-clock days, and the date horizon is evaluated vacuously every pass while the file only grows.
...
1. Run `bin/fm-startup-memory-budget.sh report` before considering a write.
   ...
   The budget is per home: this home's three files against this home's own allowance, never an agent pool total.
...
2. Read every current memory file completely: `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`.
   ...
   In a domain agent home, `data/captain-shared.md` is a read-only primary-owned input: count it, never edit it, and curate only the editable local files.
...
3. Build one whole-file retention plan before editing, ordered by likelihood of informing a future session.
   Keep in always-loaded memory only current boss preferences, authority and safety boundaries, recurring working style, agent-pool-wide or frequently relevant operating facts, and concise pointers that are expensive to rediscover.
...
7. When the total is still over budget after decay and consolidation, make aggressive reduction the default, using editable files only and in this order: archive every editable stale, superseded, or low-utility entry that is eligible for archival; consolidate tighter; run the over-budget offload sweep below and autonomously relocate every eligible non-pinned conditional entry into an already-existing allowed owner only after that owner holds it; then, only when the convergence precondition below holds, archive eligible `aging` entries oldest-reinforced-first until within budget.
   ...
   The sole exception is relocation to a JIT owner after explicit, per-item boss approval under the offload flow below, and that entry remains in memory until its destination is live.
8. Run `bin/fm-startup-memory-budget.sh report` again after the complete pass.
   Finish at or below the effective budget, or open a concrete boss decision before ending the pass.
   A domain agent must explicitly report `primary-owned-shared-file-alone-exceeds-budget` when the inherited shared file alone exceeds its allowance, because local curation cannot resolve it.
   Route that constraint to the primary owner and open one concrete boss decision at the primary owning level that names the shortfall, with exactly these options: raise the affected home's effective budget, or explicitly approve the primary owner trimming or offloading each named shared-file entry.
   ...
   Only after every safe non-pinned archival, consolidation, offload, and eligible eviction action is exhausted may a remaining excess be attributed to pinned safety, authority, or genuine boss-preference entries.
   In that last-resort case, create one boss-held decision that names the shortfall and each relevant pinned entry, with exactly these options: raise the effective budget, or explicitly approve offloading or trimming a named pinned entry.
   Route a read-only ownership constraint to its primary owner, and make every other unresolved excess a concrete boss decision that names the safe action still required.
...
Each home keeps its own archive, the archive never cascades, and truncating a grown archive is a boss decision, not a mechanism.

## Over-budget offload to JIT-loaded owners

Decay handles staleness over time; offload handles scope: knowledge that is current and durable but relevant only in a nameable context, and therefore wrong to pay for in every session of every agent pool member.
...
3. Keep, the default outcome for this sweep: current, durable, and either agent-pool-wide-relevant or safety-relevant even in sessions that never name the topic.
...
- Eligible by authority: only a non-pinned, dated `aging` entry that is not pending offload may be autonomously relocated to an already-existing allowed owner, while a `pinned` entry may be proposed only for explicit, per-item boss-approved relocation and can never be archived or autonomously offloaded for budget relief.
...
**Hard rule: the stow process never creates or writes a Synapse-repo-tracked skill.**
**Every skill stow's offload produces for a Synapse home is user-owned and local, excluded through that active home's repository-local exclude file resolved with `git -C "$home_root" rev-parse --git-path info/exclude`; contributing a lesson to the shared tracked template is a separate deliberate boss action, never automatic.**
...
- A user-owned local skill: a directory under `.agents/skills/<freeform-name>/` whose path is appended to the active home clone's repository-local exclude file, never to a `.gitignore`.
  Resolve `home_root` to `$FM_HOME` when it is set and otherwise to the Synapse code root, and anchor every destination index check, exclude-path lookup, and ignore verification to that root with `git -C "$home_root"`.
  ...
  The name is freeform with no user-vs-Synapse naming convention, the skill stays per-home and untracked, and the harness still lists and JIT-loads it because skill discovery scans the filesystem and ignores git status (verified in `docs/verification/stow-memory.md`).
...
- A project's existing committed `AGENTS.md`, for project-intrinsic knowledge useful to nearly every session of that project, through a normal agent execution task using `bin/fm-ensure-agents-md.sh` and the project's registered delivery mode.
- A project-level skill in the project's own repository, for situation-conditional knowledge within one project, through the same execution-task path.

Forbidden destinations: any Synapse-repo-tracked skill per the hard rule; Synapse's own `AGENTS.md`, which is always-loaded for every agent pool session; `docs/` alone, which is never agent-loaded on demand, though a skill body may point into docs for depth; and any committed surface for private content.
...
2. Propose pinned relocation only.
   For a pinned candidate, append a `proposed-offload` section with the same fields to the completion receipt and create or refresh one durable boss-held backlog item using `tasks-axi add`, `tasks-axi hold`, `tasks-axi show <id> --full`, and `tasks-axi update <id> --body-file <path>` as appropriate.
   ...
   If the boss never answers, nothing migrates and the held item persists, but it is never treated as budget relief.
3. Migrate an approved pinned candidate outside this pass.
   Resolve `home_root` to `$FM_HOME` when it is set and otherwise to the Synapse code root, then re-validate the approved local-skill destination under that root for both index absence with `git -C "$home_root"` and filesystem collision absence.
...
1. **Sweep the session for uncaptured durable knowledge.**
   Look for operational learnings, boss preferences expressed in passing, project-intrinsic facts, standing decisions, and undone next steps.
...
3. **Write within the existing boundaries.**
   ...
   - Project-intrinsic knowledge never goes directly into a project's `AGENTS.md`.
     Route it through a normal execution task so an agent records it with `bin/fm-ensure-agents-md.sh` and the project's delivery path.
   - Knowledge general to every Synapse user belongs in this repo's shared tracked material through the normal branch, self-review, PR, and boss-merge path.
...
4. **Use inspect-then-update.**
   ...
   The only graduation moves are promotion to tracked shared material through a PR, folding a learning into the boss-preference destination selected by AGENTS.md, archiving a stale entry to `data/memory-archive.md`, autonomous offload of an eligible non-pinned conditional entry to an already-existing allowed owner through the reduce flow above, boss-approved offload of a pinned durable conditional entry to a JIT-loaded owner executed through the migration step above, or deletion of an entry that is a duplicate or already preserved through a stronger existing owner.
...
Report the outcome in plain boss-facing language with all of these facts:
...
## Automatic cascade to domain agents

In a primary home, every `/stow` cascades to every registered domain agent after this home's own required pass and knowledge sweep are complete.
In a domain agent home, `/stow` curates that home only and never cascades further.
...
It enumerates each registered domain agent exactly once, reports that home's own budget accounting, and resolves how the sweep reaches it; its header owns the stanza fields, the bound, and the exit codes.
...
- `agent` - send the marked request with `bin/fm-send.sh fm-<id> "<request>"` so the live domain agent performs its own `/stow`, including the uncaptured knowledge that exists only in its session.
...
- `direct` - curate that local home's editable memory files yourself under the same retention plan, then re-run the cascade to confirm the after totals.
  `data/captain-shared.md` stays a read-only counted input there, exactly as it is in any domain agent home.
- `deferred` - a remote home with no live agent. Its memory is accounted read-only and cannot be curated from here, because there is no generic remote write path for a home's own memory files.
  ...
  Relaunching that domain agent is a separate decision owned by `secondmate-provisioning`, never something `/stow` does on its own.
...
A newly discovered shared boss preference still routes to the primary's `data/captain-shared.md` under the existing primary-authoritative contract, whichever home found it.
...
Extend the completion receipt with one entry per domain agent alongside the primary's own, carrying that home's budget before and after, its per-file actions, its exceptions, and whether that home swept itself or was curated from here.
Keep those entries in the same plain boss-facing language the rest of the receipt uses.
...
The exclusion binds the pass as a writer: proposing an offload and letting the migration step execute a boss-approved candidate later is not the pass storing a skill.
Every Synapse-home skill that migration produces is user-owned and local under the destinations hard rule, while an approved project-level destination is produced and shipped through that project's registered delivery path, never by stow.
Changing Synapse's tracked `.agents/skills/` or public `skills/` remains a deliberately scoped Synapse repository task through its pipeline, never a stow product.
Outside a boss-approved offload, generalizable knowledge still routes to shared tracked material through its pipeline and agent-pool-local knowledge to `data/`.
