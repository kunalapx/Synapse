...
3. **Write within Synapse's existing write boundaries.**
   ...
   - Knowledge generalizable to every Synapse user: this repo's own `AGENTS.md` (or other shared, tracked material), shipped through the normal branch -> no-mistakes -> PR -> boss-merge pipeline for this repo (section 1), never hand-committed straight to `main`.
